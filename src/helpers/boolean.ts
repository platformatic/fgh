...
export const handleDefault = (left: any, right: any): any => {
  ...
  if (Array.isArray(left)) {
    // Empty arrays from 'empty' should trigger default behavior
    // We can check if this array was generated by the 'empty' keyword by checking for a special property
    // that we'd define in generateEmpty
    if (left.length === 0 && (left as any)._fromEmpty) {
      return right
    }

    // Regular empty arrays should not trigger default behavior
    if (left.length === 0) {
